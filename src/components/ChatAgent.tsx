...
export function ChatAgent() {
  ...
  const latestSuggestions = useMemo(() => {
    ...
    return assistantMessages.find((msg) => msg.suggestions)?.suggestions ?? [];
  }, [messages]);
    ...
    <div className=\"chat-shell\">
      <div className=\"chat-header\">
        ...
        <div>
          ...
          <p className=\"chat-subtitle\">Always-on help for frames, lenses, fittings & care</p>
        </div>
      </div>
        ...
        {messages.map((message) => (
          ...
          >
            {message.content.split(\"\\n\").map((chunk) => (
              <p key={chunk.slice(0, 12)}>{chunk}</p>
            ))}
